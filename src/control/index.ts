...
export default class Control {
  ...
  mousedownHandler = (e: MouseEvent) => {
      ...
      case 0:
        {
          if (block && block.object instanceof THREE.InstancedMesh) {
            ...
            //intersected block position
            const position = new THREE.Vector3().setFromMatrixPosition(matrix)

            // don't remove bedrock
            if ( (BlockType[block.object.name as any] as unknown as BlockType) === BlockType.bedrock) {
            ...
            const mesh = new THREE.Mesh(
              new THREE.BoxGeometry(1, 1, 1),
              this.terrain.materials.get( this.terrain.materialType[parseInt(BlockType[block.object.name as any])])
            )
            ...
            // Animation to remove obstacles
            const animate = () => {
              if (performance.now() - time > 25) {
            ...
            for (const customBlock of this.terrain.customBlocks) {
              if (customBlock.x === position.x && customBlock.y === position.y && customBlock.z === position.z) {
                existed = true
     ...
     * 
     * 
     * NOTE: 1 TIME CLICK EVENT TO ISSOLATE WORK
     * 
     * 
     */
  ...
  wheelHandler = (e: WheelEvent) => {
    if (!this.wheelGap) {
      this.wheelGap = true
      setTimeout(() => {this.wheelGap = false}, 100)
      if (e.deltaY > 0) {
        this.holdingIndex++
